...
class User extends MongoModels {
    ...
    async checkCanConfirm() {
        ...
        if (this.roles.account) {
            const account = await Account.findById(this.roles.account.id);
            return account.hasPermissionTo("canConfirm");
        } else if (this.roles.admin) {
            const admin = await Admin.findById(this.roles.admin.id);
            return (admin != null);
        } else {
	    return false;
	}
    ...
    async checkCanLogin() {
        ...
        if (this.roles.account) {
            const account = await Account.findById(this.roles.account.id);
            return account.hasPermissionTo("canLogin");
        } else if (this.roles.admin) {
            const admin = await Admin.findById(this.roles.admin.id);
            return (admin != null);
        } else {
